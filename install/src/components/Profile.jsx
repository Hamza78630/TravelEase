...
const Profile = () => {
    ...
    const fetchData = async () => {
      try {
        const [profileRes, bookingsRes] = await Promise.all([
          fetch('https://travelease-production-eec5.up.railway.app/users/profile', {
            headers: { 'Authorization': `Bearer ${token}` }
          }),
          fetch('https://travelease-production-eec5.up.railway.app/bookings/mine', {
            headers: { 'Authorization': `Bearer ${token}` }
          })
        ]);
  ...
  const handleLogout = async () => {
    const token = localStorage.getItem('token');
    try {
      await fetch('https://travelease-production-eec5.up.railway.app/users/logout', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
    } catch (err) {
  ...
  const handleCancel = async (bookingId) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`https://travelease-production-eec5.up.railway.app/bookings/cancel/${bookingId}`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` }
      });
